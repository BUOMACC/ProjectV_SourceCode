...
>   2.2 [선입력 처리](#input_02)  
> 3. [멀티 플레이 위치 & 회전 동기화](#cmc_network)  
...
`Unreal Engine 5`으로 넘어오면서 `Gameplay Ability System(GAS)`이라는 기능을 본격적으로 활발하게 지원되기 시작했고, 이는 멀티플레이 환경을 고려하여 개발된 시스템이었기 때문에 프로젝트와 가장 부합하다고 판단했습니다.  

업계에서도 `GAS` 사용에 대한 경험을 중요하게 여기는 경우가 적지않게 있었기 때문에 배우면서 프로젝트에 적용하고자 했습니다.
...
`GAS`는 '무언가 한다'라는 동작을 `Ability` 단위로 관리하는 방식으로 굉장히 추상적인 개념이었기 때문에 제일 먼저 아래와 같은 상황들을 나열하고 고민했습니다.    
...
행동들을 개별 `Ability`로 나뉘게 되면 관리가 어려워지고 하나로 합치면 의미가 모호해지는 문제가 있었습니다.	
> 단순히 `피격`이라는 이름을 보면 어떤 피격인지 대해 명확하게 알 수 없음
 
그렇기 때문에 각각 별도의 `Ability`로 나눈 다음 합칠 수 있다면 합치는 방향으로 작업했습니다.    

아래와 같은 행동들이 위의 내용에 대한 예시입니다.
- 공격(콤보)    
  + 흐름이 자주 변경될 때가 많으며 중간 동작을 사용하는 경우도 있음    
    → 어떤 행동 이후에 콤보의 일부부터 시작하는 경우가 해당    
- 에어본
  + Start -> Loop -> End의 흐름에서 벗어나는 일이 거의 없으며, 모든 상황에서 실행 순서가 동일함 (Start -> Loop -> End)    

`Ability`가 많아지기 때문에 관리가 힘들 수 있다는 단점이 있지만, 개별 `Ability`로 관리되기 때문에 단위가 명확해지고 재사용에 용이했습니다.	   

실제로 공격과 관련된 행동들은 개별 `Ability`로 관리했는데, 하나의 `Ability`로 묶어서 관리하게 되면 `Ability`를 연결하는 로직을 아래와 같이 두 가지 방식으로 분리해야 했습니다.   
...
이런 방식이 `Ability`를 개별 행동 단위로 분리하려는 의도에도 적합했습니다.
...
`Ability`에서 다른 `Ability`로 분기될 때 Input이나 캐릭터의 상태처럼 다양한 상황에서 처리될 수 있도록, 조건을 만족하면 실행하는 구조로 설계했습니다.  
또한 이후 확장성을 고려하여 실행되는 `로직(Action)`과 `조건(Condition)`을 객체화하여 시스템을 구했습니다.  
설정한 Start - End 범위에서 지속적으로 체크하고 `조건(Condition)`을 만족하면 다음 `로직(Action)`을 실행합니다.

구현할 때에는 `AbilityTask`와 `AnimNotifyState` 두 가지의 방법중 `AnimNotifyState`를 이용해 구현했습니다.  
하나의 `Ability`내에서 다른 `Ability`로 분기되는 지점이 많기에 `AbilityTask`로 구현하게 되면 서로의 타이밍을 비교해 구성하기 어려울 것이라고 판단했습니다.	
> `AbilityTask` : A, B, C, D...	→ 누가 먼저 실행되는지, 순서를 잘못 지정했는지 파악하려면 모든 `AbilityTask`의 Start - End 범위를 비교해야 함
> `AnimNotifyState` : 어떤 순서대로 먼저 실행되고 언제 끝나는지 Start - End를 시각적으로 바로 파악할 수 있음

그 밖에도 `AbilityTask`는 인스턴스화되어 런타임에 내부 변수를 저장할 수 공간이 있다는 장점과, 비동기 방식으로 실행되는 강점이 있지만  
`AnimNotifyState`를 통해 처리하는 방식으로 얻는 이점이 사용하지 않는 `AbilityTask`의 일부 기능을 잃는 것보다 더 크다고 판단하여 `AnimNotifyState`로 결정했습니다.  
> 구현하고자 하는 시스템은 내부에 데이터를 저장하지 않아도 구현이 가능했고 비동기 방식으로 처리할 이유가 없었기 때문임

결과적으로 엔진에서 행동들을 연결하는 작업을 하면서 내부 코드를 수정하지 않고도 `Condition` 및 `Action`의 확장으로 유연하게 대처할 수 있었습니다.
...
`HealthAttributeSet`의 데이터를 `CombatAttributeSet`에 포함하지 않은 이유는, 전투 기능 없이 피해만 받는 Actor가 생길 수 있기 때문입니다.  
...
AttributeSet에서 데이터를 추가할 때 이 데이터는 어떻게 관리되어야 할 것인가에 대해 초점을 두고 작업했습니다.  
...
`PreAttributeChange()`에서만 Clamping 할 경우 → `BaseValue`가 비정상적인 값으로 변경되는 걸 막을 수 없으므로 두 곳에서 Clamping을 처리해 주었습니다.		

예를 들어, 공격력이 20인 상태에서 50만큼 감소하고 40만큼 증가하는 상황이 있다고 했을 때

플레이하는 유저는 "공격력의 최소가 0이니까 40이겠네" 싶지만 아래의 흐름에선 그렇지 않다는 것을 볼 수 있습니다.	
...
(1) 번의 방식은 `ID`를 사용한 매핑 방식으로 다른 개발자의 입장에서 입력값을 파악하기 힘들 것 같다는 우려가 있었습니다.    
...
이런 기능은 "키를 눌렀는데 왜 안 나가지?" 같은 플레이 경험을 개선하기 위한 기능이었으며 아래와 같이 동작합니다.	
...
- 여러 Task가 동시에 동작하는 상황에서 특정 Task가 먼저 실행되어야 하는 경우 각 Task의 실행 시간을 일일이 확인하고 값을 기입하는 번거로움이 줄어듬
...
`Server` / `Client`의 프레임이 각각 다르고 지연시간이 커지는 경우 오차가 누적되어 잘못된 움직임으로 판단되어 보정이 발생했습니다.  
즉, 위 코드는 `Client` / `Server`의 환경을 고려하지 않은 채로 각각 따로 수행되면서 발생하는 코드였습니다.
...
`PhysicsRotation( )` 내부에서 설정된 값을 활용해 회전을 처리해 주는 방식으로 처리했습니다.
...
확인해 본 결과 서버에서 한 번 더 회전 연산을 하고 있었기에 발생했던 문제였습니다.  
다른 모든 회전 값들은 정상적으로 동기화되는 반면, 마지막 한 번의 연산으로 보정 현상이 발생했기 때문에  
...
원하는 용도로 사용할 수 있게 만들어진 `FLAG_Custom`을 활용해서 동기화된 Flag는 `bWantsToRootMotionRotation` 변수에 저장하여 회전을 사용하고자 할 때 설정해 주었습니다.
...
> 원인을 파악하는 데 시간은 걸렸으나, 해결은 비교적으로 단순했습니다.    
  Listen Server의 경우 부드럽게 회전하기 위해 최종 값이 아닌 보간 값을 사용하기 때문이었는데     
...
지금까지 작업한 내용은 사용하고자 할 때 목표 회전 값과 회전량을 `Client` / `Server` 각각에서 설정했기 때문이었고       
지속적으로 변화하기 위해서는 이 두 개의 값 역시 `Client`의 환경을 고려하여 `Server`에 값을 동기화해주어야 했습니다.        
...
- 장비를 교체할 때 동일한 타입으로 부여된 `AbilitySet`의 `Handle`을 찾을 방법이 필요
  > Sword를 착용한 상태에서 Katana로 교체 시, Sword로부터 부여된 Ability를 회수해야 함    
  > 장비 객체에 Handle을 저장하는 방식으로 해결되지만, 다른 타입(포션, 스킬) 처리를 위해 중복된 코드를 작성해야 할 수도 있음
- 회수할 때 `Ability`가 이미 실행 중인 경우에는 회수해야 할지 정책이 필요   
  > A 스킬을 사용하는 중에 회수되면 Ability가 조기 종료되어 의도되지 않게 다음 동작이 가능해짐
...
UI 입력을 플레이어의 가장 상단 위젯부터 순차적으로 처리할 수 있는 `Stack` 기반 구조를 목표로 설계하고자 했습니다.		

Unreal Engine 5에서 위 구조를 기반으로 처리되는 `Common UI` 플러그인을 제공하고 있었기에 적용하였으며 의도한 흐름대로 처리할 수 있었습니다.	

플러그인을 프로젝트에 적용하는 과정에서 코드를 분석하면서 엔진이 추구하는 UI 입력 처리 흐름에 대해서 배울 수 있었습니다. 	
...
시스템 UI들은 게임 UI보다 항상 위에 보여야 하므로 이를 관리하기 위해 `Layer` 방식으로 구현했습니다.	
> 다수의 게임들을 보면 인벤토리 같은 게임 UI들은 활성 순서에 관계없이 MessageBox보다 뒤에 있어야 하기 때문임


이를 구현하기 위해 카테고리에 따라 `Z-Order`가 관리되도록 `UI Layer`를 가장 기반으로 배치했고 `ActivatableWidget`들은 적합한 `UI Layer`에 쌓이게 됩니다.
...
빠르게 타입을 추가할 수 있도록 `Component`의 Detail 패널을 통해 시작 시 등록할 퀵슬롯 타입(`FRegisteredQuickSlotInfo`)을 지정할 수 있습니다.    
등록된 퀵슬롯은 `FQuickSlot` 구조체로 저장되어 어떤 아이템을 들고 있는지, 어떤 인덱스가 활성화되어 있는지 정보를 가집니다.
...
- 상호작용 가능한 물체에 다가갔을 때 상호작용 정보

**공통적이지 않은 부분**    
- 상호작용 시 물체마다 반응하는 동작
- 물체가 상호작용 가능한 상태인지 판단하는 로직
...
- 스캔 된 적이 마무리 일격이 가능한 상태일 경우, `DeathBlow Executor Ability`가 플레이어에게 부여됩니다.
...
맵에 원하는 몬스터를 규칙에 맞게 스폰 할 수 있도록 스포너를 구현했습니다.    
어떤 몬스터가 스폰 될지 가중치를 설정하거나 어떤 스팟에 스폰 될지 설정할 수 있습니다.
...
그 밖에도 소리를 들었을 때 발생한 Noise를 감지하거나 데미지를 받은 경우 어그로를 변경하도록 구현했습니다.    

위와 같은 동작에는 `Blackboard`의 `SenseValue(float)` 및 `LastSenseActor(Actor)`값 두 개의 변수로 제어됩니다.    
