...
>   1.2 [전투 구조](#combat_02)  
>   1.3 [공격 / 피격 처리](#combat_03)  
>   1.4 [데미지 프로세스](#combat_04)  
> 3. [입력 처리](#input)  
...
GAS는 '무언가 한다' 라는 동작을 `Ability` 단위로 관리하는 방식으로 굉장히 추상적인 개념이었기 때문에 제일먼저 아래와 같은 상황들을 나열하고 고민했습니다.    
...
Ability가 많아지기 때문에 관리가 힘들 수 있지만 개별 `Ability`로 관리되기 때문에 오히려 행동을 구별하는데 있어 직관적이었습니다.    
'기본공격'이라는 이름보다 '기본공격A', '기본공격AA'라는 이름이 다른 사람이 인지하기에 직관적이라고 생각했기 때문입니다.    

<details>
<summary>가공 전 내용 접기 / 펼치기</summary>
    
**전투 시스템의 목표**
- 전투와 관련된 행동들을 추가할때 별도의 코드작업 없이 추가할 수 있어야 함    
- 핵심적인 전투 시스템의 수정을 최소하고 쉽게 확장할 수 있는 방식이어야 함    
  → 기능들을 끼워넣는 방식(Task)하면 이 장점이 극대화될 수 있음    
- 다른 사람이 내부 로직을 자세히 들여다보지 않아도 동작을 예상할 수 있어야 함    


하나의 Ability로 묶었을때의 장점
- 일단 관리하기가 용이함 -> 당연히 하나의 Ability니까.

하나의 Ability로 묶었을때의 단점
- '다음 콤보로 이어가라' 라는 행위와 '다른 어빌리티로 분기해라' 라는 행위가 분리됨
  그말은 즉슨, 여러개의 `AnimMontage`들이 하나의 콤보에 종속되는 현상이 생긴다는 점임
  만약 기존에 한 콤보에 묶여있던 `AnimMontage`가 다른 타입으로 분리된다면, 해당 타입의 Ability로 추가해야 함


개별 Ability로 쪼갰을때의 장점
- 개별 행동으로 관리되기 때문에 원하는 액션을 직관적으로 사용할 수 있음
  > 프로젝트에서 실제로 n번째 기본공격으로 이어질 수 있다는점은 재사용 측면에서 큰 장점
- 행동 단위로 나누어졌기 때문에 '다른 어빌리티로 분기해라' 라는 행위를 통합하여 사용할 수 있음
- 만들어둔 로직을 재사용할 수 있다.

개별 Ability로 쪼갰을때의 단점
- 관리하기가 어려움 -> Ability가 많아질 우려 발생

개별로 Ability를 관리하게 되면 동일한 흐름을 관리할 수 있게 된다는 장점이 있습니다.

'다음 콤보로 이어가라' / '다른 `Ability`로 분기해라' 처럼 2가지 행위로 분리되는 방식보다    
'다른 `Ability`로 분기해라' 라는 방식으로 통합하면 하나의 기능만 알아도 두 가지 상황을 모두 커버할 수 있다는 장점이 있습니다.

또 개인 프로젝트에서는 공격의 일부 Ability를 다른 행동에서 분기하는 방식이 많았기 때문에
> 패링 후속 공격 다음이 기본공격의 3타일때
최대한 Ability를 분리하되, 하나의 흐름으로 볼 수 있는 경우에는 하나의 Ability로 관리하고자 했습니다.
결과적으로 Ability를 적재적소에 맞게 원하는 조합하는 방식을 구현할 수 있었습니다.

그렇기 때문에 공격과 관련된 행동들은 개별 Ability로 관리하고
피격(Airborne_Start -> Loop -> End, SmallHit(Directional))에 대해서는 하나의 Ability로 관리하도록 했습니다.
...
'다음 콤보로 이어가라' / '다른 `Ability`로 분기해라' 처럼 2가지 행위로 분리되는 방식보다    
'다른 `Ability`로 분기해라' 라는 방식으로 통합하면 하나의 기능만 알면 위의 두 가지 상황을 모두 커버할 수 있다는 장점이 있습니다.    
