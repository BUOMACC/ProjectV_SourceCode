...
`Attribute`의 값을 제한하기 위해 `PreAttributeBaseChange()`와 `PreAttributeChange()`  두 함수를 사용했습니다.

<details>
<summary>가공 전 접기 / 펼치기</summary>
  
`BaseValue` 및 `CurrentValue` 값 제한이유?
BaseValue는 기본값이 바뀌는 경우로 예를들어 공격력이 무기에 의해 베이스값으로 증가
반면에 CurrentValue는 공격력 증가같은 버프를 통해 증가할 수 있어 CurrentValue를 계산하는데 적용됨
두 함수를 통해 제한하지 않으면, 외부적 요인과 베이스 요인에 의한 클램핑 처리를 제대로 할 수 없음
</details>
