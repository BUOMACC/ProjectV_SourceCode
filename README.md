...
`Ability`가 많아지기 때문에 관리가 힘들 수 있다는 단점이 있지만, 개별 `Ability`로 관리되기 때문에 오히려 행동을 구별하는 데 있어 직관적이라고 생각했습니다.   
'기본 공격'이라는 이름보다 '기본 공격 A', '기본 공격 AA'라는 이름이 다른 사람이 인지하기에 직관적이라고 판단했기 때문입니다.    
