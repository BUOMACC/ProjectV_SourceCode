...
구현할 때에는 `AbilityTask`와 `AnimNotifyState` 두 가지의 경우로 생각해 볼 수 있었습니다.  
하나의 `Ability`내에서 다른 `Ability`로 분기되는 지점이 많기에 `AbilityTask`로 구현하게 되면 서로의 타이밍을 비교해 구성하기 어려울 것 같다고 판단했습니다.
...
결과적으로 엔진에서 행동들을 연결하는 작업을 하면서 변화에 의한 내부 코드를 수정하지 않고도 유연하게 대처할 수 있었습니다.
...
가장 상단의 UI부터 순차적으로 처리되는 `Stack` 구조를 목표로 구현하고자 했으며 `Common UI` 플러그인이 이를 기반으로 처리되도록 지원하고 있었기에 적용했습니다.    
...
카테고리에 따라 `Z-Order`가 관리되도록 `UI Layer`를 가장 기반으로 배치했고 `ActivatableWidget`들은 적합한 `UI Layer`에 쌓이게 됩니다.
